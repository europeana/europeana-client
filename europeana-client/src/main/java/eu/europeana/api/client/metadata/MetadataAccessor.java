...
public class MetadataAccessor {
	...
	public MetadataAccessor(Api2QueryInterface query,
	 ...
	 */
	protected void fetchBlock(int edmField, int blockStartPosition,
			int blockLimit) {
		try{
	...
	void handleException(EuropeanaApiProblem e) {
		if(ERROR_POLICY_RETHROW == errorHandlingPolicy)
			throw new TechnicalRuntimeException("Api invokation error!",  e);
		else if(ERROR_POLICY_IGNORE == errorHandlingPolicy)
			log.trace("Server Error occured and ignored: " + e.getMessage());
	 ...
	 */
	protected void fetchNextBlock(int edmField, int start, int limit) throws TechnicalRuntimeException, EuropeanaApiProblem{
